...
//            this.password = '';
        ...
        renderModalLoginForm() {
            if (document.querySelector('.modal-login .modal-body p')) {
                ...
                document.querySelector('.modal-login .modal-body p').style.display = 'none';
            }
    ...
    class Register {
        ...
        init() {
            ...
            this.validatePasDuplicate();                        // проверка что пароли совпадают
            this.validateDataBirth();                           // проверка что дата рождения введена
        ...
        clickRequestRegistration() {
            ...
            document.querySelector('.modal-login .modal-footer .request_registration').onclick = () => {
                ...
                else if (!this.data['date_of_birth']) {
                    // если дата рождения не введена
                    document.querySelector('#defaultForm-date_of_birth').style.borderColor = '#f44336';
                    this.renderErrorRegistration('Введите дату рождения.');                         // вывод предупреждения
                }
                else {
        ...
        requestRegister() {
            ...
            .then(data => {
                ...
                if (data.detail && data.detail === 'Verification e-mail sent.') {
                    ...
                    this.renderModalVerification();                                     // отрисовка содержимого модального окна при удачной регистрации
        ...
        validateEmail(email) {
            ...
            return /^([a-z0-9_\.-]+)@([a-z0-9_\.-]+)\.([a-z\.]{2,6})$/.test(mail)
        }
    }
